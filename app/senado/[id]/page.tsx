...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${c.name} — Candidato al Senado 2026 | VotoAbierto`,
    description: `Conoce la hoja de vida, antecedentes y bienes declarados de ${c.name}, candidato al Senado por ${c.party}. Elecciones Perú 2026.`,
    openGraph: {
      title: `${c.name} — Senado 2026 | VotoAbierto`,
      description: `Candidato al Senado por ${c.party}. #${c.listPosition} en lista.`,
      url: `https://votoabierto.pe/senado/${c.id}`,
    },
    alternates: { canonical: `https://votoabierto.pe/senado/${c.id}` },
  }
}

export default async function SenadoCandidatePage({ params }: Props) {
  const { id } = await params
  const c = candidates.find((x) => x.id === id)
  if (!c) notFound()
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="max-w-5xl mx-auto px-4 pt-6">
        <Link href="/senado" className="text-sm text-[#1A56A0] hover:underline">
          &larr; Volver al Senado
        </Link>
      </div>
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-xs px-2 py-0.5 rounded-full bg-[#EEEDE9] text-[#555555] font-medium">Senado Nacional</span>
      ...
      <div className="max-w-5xl mx-auto px-4 pb-10 space-y-12">
        ...
        <FeedbackWidget pageUrl={`/senado/${c.id}`} />
      </div>
    </main>
  )
}
