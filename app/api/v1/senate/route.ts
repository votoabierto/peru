...
import senateData from '@/data/senate-candidates.json'

interface SenateCandidateJSON {
  id: string
  name: string
  party: string
  partyId: string
...
export async function GET(request: NextRequest) {
  ...
  let candidates = senateData as SenateCandidateJSON[]
  ...
  const clean = paginated.map((c: SenateCandidateJSON) => ({
    id: c.id,
    name: c.name,
    party: c.party,
    partyId: c.partyId,
