...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${c.name} — Diputado por ${c.district} 2026 | VotoAbierto`,
    description: `Conoce la hoja de vida, antecedentes y bienes declarados de ${c.name}, candidato a diputado por ${c.district}. Elecciones Perú 2026.`,
    openGraph: {
      title: `${c.name} — Diputado ${c.district} 2026 | VotoAbierto`,
      description: `Candidato a Diputado por ${c.district}, partido ${c.party}. #${c.listPosition} en lista.`,
      url: `https://votoabierto.pe/diputados/${toDistritoSlug(c.district)}/${c.id}`,
    },
    alternates: { canonical: `https://votoabierto.pe/diputados/${toDistritoSlug(c.district)}/${c.id}` },
  }
}

export default async function DiputadoCandidatePage({ params }: Props) {
  const { distrito, id } = await params
  const c = candidates.find((x) => x.id === id && toDistritoSlug(x.district) === distrito)
  if (!c) notFound()
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="max-w-5xl mx-auto px-4 pt-6">
        <Link href="/diputados" className="text-sm text-[#1A56A0] hover:underline">
          &larr; Volver a Diputados
        </Link>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 pb-10 space-y-12">
        ...
        <FeedbackWidget pageUrl={`/diputados/${distrito}/${c.id}`} />
      </div>
    </main>
  )
}
